...
async def verify_dispute_sorting():
    client = WikiClient()
    
    print("\n--- Testing Sort by DATE (Last Updated) ---")
    results = await client.get_top_talk_pages(limit=10, sort="date")
    ...
    print("\n--- Testing Sort by COUNT (Most Active) ---")
    results = await client.get_top_talk_pages(limit=10, sort="count")
...
if __name__ == "__main__":
    asyncio.run(verify_dispute_sorting())
